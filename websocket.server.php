...
class Storage extends SplObjectStorage {
	public function offsetGet( $findObject) {
		$key = $this->key();
...
class WebSocketServer implements WebSocketObserver{
	...
	public function __construct($url, $adminKey){
		...
		$this->sockets = new Storage();
		$this->_connections = new Storage();
	...
	protected function dispatchAdminMessage(IWebSocketConnection $user, IWebSocketMessage $msg){
		...
		if(array_key_exists($this->_connections->offsetGet($user),$this->uriHandlers)){
			$this->uriHandlers[$this->_connections->offsetGet($user)]->onAdminMessage($user, $msg);
		}
	...
	protected function dispatchMessage(IWebSocketConnection $user,IWebSocketMessage $msg){
		...
		if(array_key_exists($this->_connections->offsetGet($user),$this->uriHandlers)){
			$this->uriHandlers[$this->_connections->offsetGet($user)]->onMessage($user, $msg);
		}
	...
	public function onDisconnect(WebSocketSocket $socket){
		...
		try{
			if($con){
				$handler = $this->_connections->offsetGet($con);

				if($handler && isset($this->uriHandlers[$handler]) && $this->uriHandlers[$handler] != null )
